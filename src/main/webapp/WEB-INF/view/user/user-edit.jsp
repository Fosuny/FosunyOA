...
<form action="#" method="post" onsubmit="return false" id="formsubmit">
    <div class="layui-form" lay-filter="layuiadmin-form-useradmin" id="layuiadmin-form-useradmin" style="padding: 20px 0 0 0;">
      ...
      <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
          <input type="text" name="staffName" id="staffName" lay-verify="required" autocomplete="off" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
        <label class="layui-form-label">手机</label>
        <div class="layui-input-inline">
          <input type="text" id="staffPhone" name="staffPhone" lay-verify="phone" autocomplete="off" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
        <label class="layui-form-label">生日</label>
        <div class="layui-input-inline">
          <input type="text" id="date" name="staffBirthday" lay-verify="required" autocomplete="off" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
        <label class="layui-form-label">Q Q</label>
        <div class="layui-input-inline">
          <input type="text" id="staffQq" name="staffQq" lay-verify="required" autocomplete="off" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
        <label class="layui-form-label">邮箱</label>
        <div class="layui-input-inline">
          <input type="email" id="staffMailbox" name="staffMailbox" lay-verify="email" autocomplete="off" class="layui-input">
        </div>
      </div>
      ...
      <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
          <input type="text" id="staffWechat" name="staffWechat" lay-verify="required" autocomplete="off" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
        <label class="layui-form-label">角色</label>
        <div class="layui-input-block" id="roleQuery">
        </div>
      </div>
      <div class="layui-form-item">
  ...
  <script>
    $("#LAY-user-front-submit").click(function (){
      $.ajax({
        url:'${pageContext.request.contextPath}/oaIndex/userEditUpload',
      ...
      for(var i=0;i<roleArrayId.length;i++){
        $.ajax({
          url:'${pageContext.request.contextPath}/oaIndex/userEditRoleUpdate?staffId=${staffId}',
          data:{"roleId":roleArrayId[i],"staffId":${staffId}},
          type:"post"
        })
      }
      console.log(1234565678)
    })
        ...
        $.ajax({
          ...
          success:function (data){
            ...
            for(let i=0;i<data.length;i++){
              sum = i+1;
              checkbox += '<input type="checkbox" name="roleIds" class="etc" value="'+data[i].roleId+'" title="'+data[i].roleName+'">'
            }
    ...
    $(document).ready(function (){
      $.ajax({
        url:'${pageContext.request.contextPath}/oaIndex/userEditData?staffId=${staffId}',
        type:'post',
        dataType: 'json',
        success:function (data){
            ...
            $("#date").val(data.staffBirthday);
            $("#staffQq").val(data.staffQq);
            $("#staffMailbox").val(data.staffMailbox);
            $("#staffWechat").val(data.staffWechat);
            $("#staffAge").val(data.staffAge);
            $("#staffWorkage").val(data.staffWorkage);
            $("#staffStatus option[value="+data.staffStatus+"]").attr("selected",true);
            $(":radio[name='staffSex'][value="+data.staffSex+"]").attr("checked",true);
            /*$(".etc:checkbox[value=1]").prop("checked",true);*/
            $.ajax({
                url:'${pageContext.request.contextPath}/oaIndex/roleIdsbox?staffId=${staffId}',
                type:'post',
                dataType:'json',
                success:function (data){
                    for(let i=0;i<data.length;i++){
                        //异步获取选中记录中包含的lsid分类集合，遍历集合，如果集合中的值与checkbox的value值相同，则选中
                          ...
                          $(str.split(",")).each(function (i) {
                            $(".etc:checkbox[value='" + data[i].roleId + "']").prop("checked", true);
                          })
                    }
                    console.log("SUCCESS！")
                }
